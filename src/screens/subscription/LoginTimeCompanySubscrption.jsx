...
import { Alert, View, Text, ScrollView, StyleSheet, TouchableOpacity, SafeAreaView, Easing, Animated, Modal, TouchableWithoutFeedback, ActivityIndicator } from 'react-native';
...
const LoginTimeCompanySubscrption = () => {
  ...
  // Set user details directly from the passed route params
  useEffect(() => {
    if (userDetails && userDetails.user_id) {
      setName(userDetails.company);
  ...
  const getFCMToken = async () => {
    try {
      ...
      console.log("📲 [FCM Refreshed]:", token);
      return token;
    } catch (error) {
  ...
  const createUserSession = async (userId, retryCount = 0) => {
    ...
    if (retryCount < MAX_RETRIES) {
      ...
      showToast("Unable to create session. Please try again", 'error');
...
export default LoginTimeCompanySubscrption
