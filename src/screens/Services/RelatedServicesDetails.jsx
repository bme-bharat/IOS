...
const RelatedServicesDetails = () => {
  ...
  const shareProduct = async (product) => {
    try {
      if (!product?.service_id || !product?.company_id) {
        return;
      }

      const productUrl = `https://bmebharat.com/Service/${product.company_id}/${product.service_id}`;
  ...
  const closeModal = () => {
    ...
    setSelectedMedia(null); // Clear media when closing
  };
  ...
  const fetchRelatedProducts = async (product, lastKey = null) => {
    try {
      const response = await axios.post(
        `${BASE_API_URL}/getRelatedServicesByServiceId`,
        {
          command: "getRelatedServicesByServiceId",
  ...
  const specifications = product?.specifications
    ? [
      ...
      { label: 'Country of origin', value: product.specifications?.manufacturer_country },
                ...
                product.files.some(file => file.toLowerCase().endsWith('.pdf')) && (
                  ...
                  >
                    {loading1 ? (
                      ...
                      <Text style={styles.pdfText}>View service catalogue</Text>
                    )}
                  </TouchableOpacity>
                )}
              ...
              {relatedProducts.length > 0 && (
                <View style={styles.relatedProductsContainer}>
                  ...
                  <FlatList
                    ...
                    horizontal
                    showsHorizontalScrollIndicator={false}
                    keyExtractor={(item, index) => item.service_id ? item.service_id.toString() : `index-${index}`}
                    renderItem={({ item }) => (
...
export default RelatedServicesDetails;
