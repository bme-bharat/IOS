...
const RelatedProductDetails = () => {
  ...
  const openModal = (mediaUrl, type) => {
    console.log('Opening modal for:', mediaUrl, 'Type:', type); // Debugging
  ...
  const specifications = product?.specifications
    ? [
      ...
      { label: 'Regulatory & Compliance', value: product.specifications.regulatory_compliance },
          ...
          <ScrollView contentContainerStyle={{ backgroundColor: 'white', }} showsVerticalScrollIndicator={false}
            ...
            <TouchableOpacity activeOpacity={1}>
              ...
              <View style={styles.priceRow}>
                ...
                {/* {product.discount_price && <Text style={styles.discountPrice}>₹{product.discount_price}</Text>}
          {product.offer && <Text style={styles.offer}> {product.offer}% OFF</Text>} */}
              </View>

              <Text style={styles.specTitle}>Specifications </Text>

              {specifications
                .filter(spec => spec.value) // Only keep items where value exists
                .map((spec, index) => (
